...
class IDM(object):
    ...
    def __init__(self, folder, fname):
        ...
        if len(self.apath) > 0:
            if not path.exists(self.apath):
                print('The idm file doesn\'t exist in %s! Please initiate the object again! ' % (self.apath))

        # Instantiate various classes from pyhton files
    ...
    # Save to the particular path, folder
    ...
    # Apply windows and doors related Lisp scirpts
    ...
    def applywwr_new(self, wwr, wall_width_list, floor_ht, name, option=0):
      ...
      """
        ...
        if self.check_bld():
            if option == 0:
                new_path, bld = self.wwrPro.wwr2_proportional(self.building, wwr, wall_width_list, floor_ht,
                                                              self.folder, name)
            elif option == 1:
                new_path, bld = self.wwrPro.wwr2_fixed_ht(self.building, wwr, wall_width_list, floor_ht, self.folder,
                                                          name)
            elif option == 2:
    ...
    def html_report_path(self, filepath):
        # Generate html report to a particular file path
