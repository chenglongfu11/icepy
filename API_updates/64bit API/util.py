...
path_to_lib = "C:\\MUBES\\64bit\\"

command = path_to_ice + "ida-ice.exe \"" + path_to_ice + "ida.img\" -G 1"
startObj = subprocess.STARTUPINFO()
ret = subprocess.Popen(command)
pid = str(ret.pid)
#print(pid)
time.sleep(5)
#Add path_to_ice to PATH variable, is removed when program finishes
os.environ['PATH'] = path_to_ice + os.pathsep + path_to_lib + os.pathsep + os.environ['PATH']

ida_lib = ctypes.CDLL(path_to_lib + 'idaapi2.dll')
